<x-guest-layout>
    ...
    <div class="bg-white text-slate-900" x-data="{ tab: 'info' }">
        ...
        <section class="py-16">
            <div class="mx-auto max-w-6xl px-4 lg:px-1 flex gap-10">

                {{-- ========================= --}}
                {{-- SIDEBAR FIJA --}}
                {{-- ========================= --}}
                <aside class="w-56 shrink-0 sticky top-20 h-[300px]">

                    <div class="flex flex-col gap-4 justify-start h-full">

                        <button @click="tab = 'info'" :class="tab === 'info'
                ? 'bg-slate-900 text-white'
                : 'bg-white border border-slate-300 text-slate-700'"
                            class="px-6 py-3 rounded-xl font-semibold transition text-left w-full">
                            Información
                        </button>

                        <button @click="tab = 'login'" :class="tab === 'login'
                ? 'bg-slate-900 text-white'
                : 'bg-white border border-slate-300 text-slate-700'"
                            class="px-6 py-3 rounded-xl font-semibold transition text-left w-full">
                            Iniciar sesión
                        ...
                        <button @click="tab = 'register'" :class="tab === 'register'
                ? 'bg-slate-900 text-white'
                : 'bg-white border border-slate-300 text-slate-700'"
                            class="px-6 py-3 rounded-xl font-semibold transition text-left w-full">
                            Crear cuenta
                ...
                <div class="flex-1">
                    ...
                    <div x-show="tab === 'info'" x-transition>
                        ...
                        <p class="mt-4 text-slate-600 leading-relaxed">
                            Un entorno seguro con registro, verificación de email, recuperación de contraseña y panel
                            por perfiles.
                            Las empresas publican ofertas y gestionan candidaturas; los candidatos crean su perfil,
                            adjuntan CV y se inscriben;
        ...
        <footer class="border-t border-slate-200">
            <div
                class="mx-auto max-w-6xl px-6 lg:px-10 py-8 text-xs text-slate-500 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
                <div>© {{ date('Y') }} Formacom Empleo</div>
                <div class="flex gap-4">
                    <a class="hover:text-slate-700" href="https://www.formacom.es/" target="_blank"
                        rel="noopener">formacom.es</a>
